...
\ml{Solver} As mentioned in section \ref{generalterms} a solver is a program which implements an algorithm. In \edacc we store the following information about a solver:
\begin{enumerate}
 \item \ml{Solver name} A human-readable name of the solver.
 \item\ml{Solver version} The version number of the solver. The combination of name and version must be unique.
 \item \ml{Solver description} A short description of the solver.
 \item \ml{Solver authors} The list of the authors of a solver.
 \item \ml{Solver code} The sourcecode of the solver.
 \item \ml{Solver binaries} A solver can consist of different binaries, which have the same source code but differ in the compile options (eg. the architecture) or the chosen compiler version. There must be at least one solver binary.
\end{enumerate}
